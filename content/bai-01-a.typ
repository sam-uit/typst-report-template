...
- #strong[Kiểu chuỗi ký tự (Character Strings)];:
...
- Giới hạn này là #strong[8.060 bytes] (xấp xỉ 8KB).

=== Dung lượng (KB) tối đa của 1 table
<dung-luong-kb-toi-da-cua-1-table>
...
- Dung lượng tối đa của Database là #strong[524,272 Terabytes];.
...
<y-nghia-cac-table-he-thong-system-tables>
...
- `sysusers`:
  ...
  - Đổi thành #link("https://learn.microsoft.com/en-us/sql/relational-databases/system-catalog-views/sys-database-principals-transact-sql?view=sql-server-ver17")[sys.database\_principals (Transact-SQL)] ở các phiên bản SQL Server hiện đại.
- `sysservers`:
  - Chứa thông tin về các server liên kết (Linked Servers) hoặc remote servers mà SQL Server này có thể giao tiếp.
  - Đổi thành #link("https://learn.microsoft.com/en-us/sql/relational-databases/system-catalog-views/sys-servers-transact-sql?view=sql-server-ver17")[sys.servers (Transact-SQL)] ở các phiên bản SQL Server hiện đại.
- `sysxlogins`:
  - Chứa thông tin về tài khoản đăng nhập (Login account) và mật khẩu (đã mã hóa) cấp server.
  - Hoặc `syslogins`
  - Hiện nay là #link("https://learn.microsoft.com/en-us/sql/relational-databases/system-catalog-views/sys-server-principals-transact-sql?view=sql-server-ver17")[sys.server\_principals] hoặc #link("https://learn.microsoft.com/en-us/sql/relational-databases/system-catalog-views/sys-sql-logins-transact-sql?view=sql-server-ver17")[sys.sql\_logins];.

Tham khảo: #link("https://learn.microsoft.com/en-us/sql/relational-databases/system-tables/mapping-system-tables-to-system-views-transact-sql?view=sql-server-ver17")[Mapping System Tables to System Views (Transact-SQL)]

=== Các file của một Cơ Sở Dữ Liệu (DB)
<cac-file-cua-mot-co-so-du-lieu-db>
...
+ #strong[File dữ liệu chính (Primary Data File - đuôi `.mdf`):]
...
+ #strong[File nhật ký giao dịch (Transaction Log File - đuôi \``.ldf`):]
  - Ý nghĩa: Ghi lại tất cả các giao dịch (`insert`, `update`, `delete`) và các sửa đổi CSDL.
  - Dùng để khôi phục dữ liệu (restore) khi có sự cố, đảm bảo tính toàn vẹn (#emph[ACID];).
...
<so-user-co-the-connect-cung-1-thoi-diem-la-bao-nhieu>
