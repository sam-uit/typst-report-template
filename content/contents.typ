...
<cac-kieu-dia-chi-nho>


== Địa chỉ vật lý (physical address) (địa chỉ thực)
<dia-chi-vat-ly-physical-address-dia-chi-thuc>
...
<dia-chi-luan-ly-logical-address>
...
<dia-chi-tuyet-doi-absolute-address>
...
<dia-chi-tuong-doi-relative-address>
...
<thoi-diem-chuyen-doi-dia-chi-nho>
...
<execution-time-thoi-diem-thuc-thi>
...
= Phân biệt Dynamic linking và Dynamic loading
<phan-biet-dynamic-linking-va-dynamic-loading>
...
#table(
  ...
  [#strong[Tiêu chí]], [#strong[Dynamic Linking (Liên kết động)]], [#strong[Dynamic Loading (Nạp động)]], [#strong[Thời điểm liên kết/nạp]], [Liên kết thư viện khi chương trình đang chạy (runtime), thường ở lần gọi
đầu tiên], [Nạp từng phần/thủ tục của chương trình chỉ khi cần dùng], [#strong[Đối tượng chính]], [Thư viện ngoài (DLL trên Windows, .so trên Unix/Linux)], [Các module/thủ tục bên trong chương trình], [#strong[Cách hoạt động]], [File thực thi chứa stub; khi gọi hàm, stub nạp thư viện và vá địa chỉ], [Lập trình viên chủ động thiết kế: thủ tục nào được gọi mới nạp vào RAM], [#strong[Hỗ trợ của HĐH]], [Cần HĐH hỗ trợ mạnh (quản lý thư viện dùng chung, kiểm tra đã nạp chưa)], [Ít phụ thuộc HĐH, chủ yếu qua thư viện hỗ trợ], [#strong[Chia sẻ mã]], [Có -- nhiều tiến trình dùng chung 1 bản thư viện trong RAM], [Không bắt buộc (mỗi tiến trình/module tự quản)], [#strong[Lợi ích chính]], [Giảm dung lượng file thực thi, tiết kiệm RAM nhờ chia sẻ], [Giảm RAM sử dụng vì không nạp phần không dùng], [#strong[Ví dụ]], [Chương trình dùng libc.so chỉ nạp khi chạy], [Module xử lý lỗi chỉ nạp khi lỗi xảy ra]
)

== Dynamic Linking (Liên kết động)
<dynamic-linking-lien-ket-dong>
...
<co-che-phan-chia-bo-nho-co-dinh-va-phan-chia-dong>
...
#table(
  columns: (1fr,) * 3,
  align: (left, left, left),
  [#strong[Tiêu chí]], [#strong[Phân chia cố định]], [#strong[Phân chia động]], [#strong[Khởi tạo và Phân chia]], [Khi khởi động hệ thống, bộ nhớ chính được chia thành nhiều phần rời nhau
gọi là các partition.], [Số lượng partition không cố định và partition có thể có kích thước khác
nhau.], [#strong[Kích thước Partition]], [Có kích thước bằng nhau hoặc khác nhau.], [Partition có thể có kích thước khác nhau.], [#strong[Cơ chế cấp phát]], [Tiến trình nào có kích thước nhỏ hơn hoặc bằng kích thước partition thì
có thể được nạp vào partition đó.], [Mỗi tiến trình được cấp phát chính xác dung lượng bộ nhớ cần thiết.], [#strong[Vấn đề phân mảnh]], [Không hiệu quả do bị #strong[phân mảnh nội] (một chương trình dù lớn hay
nhỏ đều được cấp phát trọn một partition).], [Gây ra hiện tượng #strong[phân mảnh ngoại] (kích thước không gian nhớ
còn trống thỏa mãn yêu cầu nhưng không liên tục).], [#strong[Xử lý khi không đủ chỗ]], [Nếu chương trình có kích thước lớn hơn partition thì phải dùng cơ chế
#strong[overlay];.], [Có thể dùng cơ chế #strong[kết khối (compaction)] để gom lại thành vùng
nhớ liên tục.]
)
...
<phan-biet-phan-manh-ngoai-vs-phan-manh-noi>
