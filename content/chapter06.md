...
1. **Khác nhau giữa mã hoá đối xứng và bất đối xứng?**
    ...
    - Trong khi đó, mã hoá bất đối xứng (Asymmetric Key) sử dụng một cặp khóa gồm khóa công khai (Public Key) dùng để mã hóa và khóa bí mật (Private Key) dùng để giải mã, tức là hai khóa khác nhau ($K_E \neq K_D$).

2. **Hash dùng để làm gì? Vì sao không thể đảo ngược?**

    - Hàm *băm* (Hash) dùng để tạo ra một "dấu vân tay kỹ thuật số" (digital digest/fingerprint) có chiều dài cố định từ một thông điệp có chiều dài bất kỳ, phục vụ cho việc chứng thực và kiểm tra tính toàn vẹn của dữ liệu.
...
6. **Chữ ký số khác mã hoá ở điểm nào?**
    ...
    - Mã hóa thông thường dùng để đảm bảo **tính bí mật** (confidentiality), người gửi sẽ dùng Khóa công khai (Public Key) của *người nhận* để mã hóa, và chỉ *người nhận* mới có Khóa bí mật (Private Key) để mở ra xem.
    - Trong khi đó, chữ ký số dùng để đảm bảo **tính chứng thực** (authentication), người gửi tự băm thông điệp và dùng Khóa bí mật (Private Key) của *chính mình* để mã hóa (ký), sau đó ai cũng có thể dùng Khóa công khai (Public Key) của *người gửi* để mở ra xác minh xem có đúng là người đó gửi hay không.
